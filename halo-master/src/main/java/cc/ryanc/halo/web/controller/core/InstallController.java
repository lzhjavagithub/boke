...
public class InstallController {
    ...
    public JsonResult doInstall(@RequestParam("blogLocale") String blogLocale,
        ...
        try {
            ...
            post.setPostTitle("Hello 程序员!");
            post.setPostContentMd("# Hello 程序员!\n" +
                    "欢迎在程序员社区进行创作，删除这篇文章后赶紧开始吧。");
            post.setPostContent(MarkdownUtils.renderMarkdown(post.getPostContentMd()));
            post.setPostSummary("欢迎在程序员社区进行创作，删除这篇文章后赶紧开始吧。");
            post.setPostStatus(0);
            post.setPostDate(DateUtil.date());
            post.setPostUrl("hello-程序员");
            post.setUser(user);
